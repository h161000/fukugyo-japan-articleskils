...
文章ルールの「導入の疑問と後続文」「第三者レビューサイトの表記」も執筆前に読む。筆者が作った読者の疑問は、カギ括弧内でも文脈レビューの対象とする。導入全文を含む`editorial-review.json`で前提と後続文の接続を確認し、公開原稿の第三者サイト名の残存も検査する。詳細と修正例は同参照へ集約する。
